...
void compute_local_extrema(double *sub_data, int nx, int ny, int nz, int nc, int *local_min_count, int *local_max_count, double *global_min, double *global_max) {
    for (int t = 0; t < nc; t++) {
...
int main(int argc, char **argv) {
    ...
    if (argc < 10) {
        if (rank == 0) printf("Usage: mpirun -np P ./executable input.txt PX PY PZ NX NY NZ NC output.txt\n");
        MPI_Finalize();
        return 1;
    }
    ...
    MPI_Scatter(data, sub_size * NC, MPI_DOUBLE, sub_data, sub_size * NC, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    ...
    compute_local_extrema(sub_data, sub_nx, sub_ny, sub_nz, NC, local_min_count, local_max_count, global_min, global_max);
    ...
    free(sub_data); free(local_min_count); free(local_max_count); free(global_min); free(global_max);
    if (rank == 0) { free(data); free(total_min_count); free(total_max_count); free(final_min); free(final_max); }
    MPI_Finalize();
    return 0;
}
